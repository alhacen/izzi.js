    ...
    allNodes.map(n=>{
        if(n.attributes){
            for(let i=0;i< n.attributes.length; i++){
                ...
                vars = parseVar(n.attributes[i].value.split(" "))
                if(vars.length){
                    vars.map(v=>{
...
const render = () =>{
    ...
    [...stateToAttributeMap.values()].map(obj=>{
        obj.map(elm=>{
            elm.object.setAttribute(elm.attribute, elm.renderer())
        })
    });
...
const initMagicTag = () =>{
    ...
    for(let i=0; i<allIzzis.length;i++){
        ...
        izziReturn = new Function(`return ${izziCodeTemplate.replaceAll("&gt;",">").replaceAll("&lt;","<").trim()}`)();
        // console.log(`return \`${izziCodeTemplate.replaceAll("&gt;",">").replaceAll("&lt;","<").trim()}\``)
        if(izziReturn){
            if(Array.isArray(izziReturn))
